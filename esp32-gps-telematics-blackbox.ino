/**
 * =========================================================================================
 * Project     : ESP32 High-Rate GPS/GNSS Fleet Blackbox & Inertial Dead Reckoning
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : Vehicle blackbox combining 10Hz GNSS NMEA receiver, 6-axis IMU dead reckoning, SD card circular buffer logging, and crash G-force incident capture.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-gps-telematics-blackbox";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 High-Rate GPS/GNSS Fleet Blackbox & Inertial Dead Reckoning"));
